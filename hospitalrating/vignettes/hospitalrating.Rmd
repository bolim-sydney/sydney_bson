---
title: "hospitalrating"
author: "Bolim Son"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{hospitalrating}
...
The purpose of this project is to check overall hospital performance of the U.S. hospitals by looking at two parameters: the frequency of infections at a hospital, and patient evaluations of a hospital. By doing so, this project will bring actual performace of a hospital (infections) with perceived performace of a hospital (evaluation). 

Infections will be compared to national average. Highter patient evaluation scores will mean a hospital performs better. The dataset will be cleaned to have a better format. Currently all the assessment criteria is lumped together in the large dataset. It will be grouped by city and state level to see which city or state has good quality hospitals.
...
# Raw tables saved in the package
# This is how I initially read it into R
# Please just see it as a reference
...
The clean data is saved as `hospital`. Please use `hospitalrating::hospital` to check it.
...
The clean data is saved as `infection`. Please use `hospitalrating::infection` to check it.
...
__3-4) Survey question dataset__
...
There are four clean datasets saved.
They are `infection`, `survey`, `hospital` and `survey_q`. All are saved as r data file using `use_data()` function.
...
The clean data is saved as `merged_data`. Please use `hospitalrating::merged_data` to check it.
